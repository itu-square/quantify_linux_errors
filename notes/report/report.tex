...
\lstdefinestyle{mystyle}{
  ...
  showtabs=false,                  
  tabsize=2,
  emph={ifdef,endif,config,def\_bool,boolean,depends,on,if,choice,endchoice,default},
  emphstyle={\color{red}}
}
...
There are only two features in the feature model: \texttt{A}, and \texttt{B}, 
...
program, there will be no warning, because the variable \textcode{var} gets initialized.
...
\figa
    \subfigure[The original code]{
    \lstinputlisting[language=C,firstline=3,]{code/varbug.c}
    } \qquad
    \subfigure[The code in $k_0$]{
    \lstinputlisting[language=C,firstline=3,]{code/varbug0.c}
    } \qquad
    \subfigure[The code in $k_1$]{
    \lstinputlisting[language=C,firstline=3,]{code/varbug1.c}
    } \qquad
    \subfigure[The code in $k_2$]{
    \lstinputlisting[language=C,firstline=3,]{code/varbug2.c}
    } \qquad
    \subfigure[The code in $k_3$]{
    \lstinputlisting[language=C,firstline=3,]{code/varbug3.c}
    }
...
A total of 190,000 warnings were collected from these experiments, with the 
...
Figure \ref{stablessdis} shows the distribution of subsystems within warning messages
 on the stable Linux version. In many experiment runs, 
there were multiple warnings refering  the same subsystems. These are only counted 
once.
...
subsystems, and those are not shown in the figures if they have a percentage of 0\%.
\\
...
\textbf{Observation 4:}
The subsystem, which appear the most in the warning messages, is the 
\texttt{drivers/} subsystem, followed by the \texttt{include/} subsystem.
\\

\textbf{Observation 5:}
There are zero warnings in the \texttt{security/} subsystem, and many of the smaller and infrastructure subsystems. 
\\
