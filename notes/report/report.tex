...
pollution}, and next come warning types related to variables being uninitialized.
...
The red rows are subsystems that are not a major part of the kernel 
functionality, as mentioned in  Section \ref{sec:linuxss}. These are the 
\textredmid{smaller} subsystems, and the \textreddark{infrastructure} subsystems.
\\
...
\textbf{Observation 3:}
The subsystem, which appear the most in the warnings, is by far the 
\texttt{drivers/} subsystem.
\\

\textbf{Observation 4:}
There are near zero warnings in the \texttt{security/} subsystem. 
\\
...
\figb{Comparrison of the top 10 warnings}{stablewarndis}



% TODO
% Goddammit, this was kinda wrong, because paths are like '/include/drivers/xxx',
% which would count as a 'drivers/' warning.
% SQL FIX: 
% select count(distinct config) from bugs where linux_version rlike '4.1.1' and 
% (original rlike '^kernel/' or original rlike ' kernel/' or original rlike 
% '\\nkernel/' or original rlike '\\[kernel');
