...
This is also called \emph{variability} in software and a software products of 
...
\end{equation}
            ...
            \paragraph{The Kconfig language} is the language of the feature 
                model in Linux 
(also used for other projects like BusyBox, BuildRoot, CoreBoot, Freetz and 
others)
    \cite[p. 4]{VarModSSD}
.
...
The features can be split into four different types, \emph{User features}, 
\emph{Implementation features}, \emph{Derived features}, and \emph{Capability 
features}
    \cite[p. 8]{VarModSSD}
.

\emph{User} features are features that the user chooses a value for in the 
...
\emph{Implementation} features are referenced in the source code, and will 
directly influence what code the preprocessor will select for compilation.

\emph{Derived} features are invisible to the user, and are set by other 
features using the \textcode{select FOO} syntax.

\emph{Capability} features in Linux are features, that simulate a reverse 
dependency. 
    \footnote{Should I give an example here of the capability one?}
    \footnote{How many are there of each of these}
...
\begin{itemize}
    \item \underline{\textbf{TODO}}
        \item Talk about 25\% of the configs to have warnings. And that it does 
        not matter (See notes.md)
\end{itemize}
